...
bool Frame::reparent_event(const XReparentEvent *e)
{
	...
	if(e->parent==fl_xid(WindowManager::instance())) return true;
...
bool Frame::clientmsg_event(const XClientMessageEvent *e)
{
	...
	if(a==_XA_WM_CHANGE_STATE && e->format == 32)
	{
	...
	if(e->message_type==_XA_NET_CLOSE_WINDOW)
	{
	...
	if(e->message_type==_XA_NET_WM_DESKTOP)
	{
