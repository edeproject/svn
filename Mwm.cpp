...
long mwm_functions(MwmHints *hints, XSizeHints *sh)
{
	long functions = ~0U;
...
long mwm_decors(MwmHints *hints, XSizeHints *sh)
{
	long decors = ~0U;
...
// Set modal
	if(!f->shown() && mwm_hints && (mwm_hints->flags & MWM_HINTS_INPUT_MODE) &&
			(mwm_hints->input_mode != MWM_INPUT_MODELESS))
	f->set_frame_flag(MODAL);

	return ((f->frame_flags() ^ old_frame_flags) || (f->decor_flags() ^ old_decor_flags)
			|| (f->func_flags() ^ old_func_flags));
}
