...
{ 
	XDrawRectangle(fl_display, RootWindow(fl_display, fl_screen), invertGc, px, py, pw, ph); 
}

void clear_overlay()
{
	if (pw > 0)
		draw_current_rect(); pw = 0;
}
...
void Frame::settings_changed()
{
	...
	if(!desktop() || desktop()==Desktop::current())
	{
		...
		{
												  //expose title
			XClearWindow(fl_display, fl_xid(title));
	...
	switch (newstate)
	{
		...
		default:
			...
			else if(oldstate == NORMAL)
			{
				...
				XUnmapWindow(fl_display, window_);
				title->hide();
			}									  // don't setStateProperty IconicState multiple times
			else
			{
				return;
			}
...
void Frame::shade()
{
	int title_h = Titlebar::default_height + box()->dh() + 1;
	restore_x = x();
	restore_y = y();
	restore_w = w();
	restore_h = h();
	...
	if(Frame::animate)
		::animate(x(), y(), w(), h(), x(), y(), w(), title_h);

	set_size(x(), y(), w(), title_h);
	shaded = true;
}
...
{
	if(Frame::animate)
		::animate(x(), y(), w(), h(), x(), y(), w(), restore_h);

	set_size(x(), y(), w(), restore_h);
	shaded = false;
}
...
int Frame::handle(int e)
{
	...
	switch (e)
	{
		...
		case FL_MOVE:
			...
			else if(!Fl::event_inside(title->x(), title->y(), title->w(), title->h()) )
			{
				tooltip(0);
				Fl_Tooltip::exit();
				handle_title = false;
